...
std::ostream& operator<<(std::ostream& os, const ci_string& cistr)
{
  os << cistr.c_str();
  return os;
}
std::istream& operator>>(std::istream& is, ci_string& cistr)
{
  std::string read;
  is >> read;
  cistr = read.c_str();
...
std::string operator+(const std::string& str, const ci_string& cistr)
{
  return str + cistr.c_str();
}
ci_string operator+(const ci_string& cistr, const std::string& str)
{
  ci_string plus = cistr;
  plus += str.c_str();
  return plus;
}

std::string& operator+=(std::string& str, const ci_string& cistr)
{
  str += cistr.c_str();
  return str;
}
ci_string& operator+=(ci_string& cistr, const std::string& str)
{
  cistr += str.c_str();
  return cistr;
}
